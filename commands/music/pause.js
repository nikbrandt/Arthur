exports.run = (message, a, s, d, permLevel) => {
	...
	if (!message.guild.music || !message.guild.music.queue || !message.guild.me.voice || !message.guild.me.voice.connection || !message.guild.me.voice.connection.dispatcher) return message.channel.send(message.__('no_music_playing'));

	if (message.guild.music.playing === false) return message.channel.send(message.__('already_paused'));

	message.guild.me.voice.connection.dispatcher.pause(true);
