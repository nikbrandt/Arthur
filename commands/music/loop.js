exports.run = (message, a, s, c, permLevel) => {
	...
	if (message.guild.music.loop) {
		...
		message.channel.send(`A\'ight ${message.member.displayName.replace(/@/g, '@\u200b')}, loops off, as you wish.`);
	} else {
		message.guild.music.loop = true;
		message.channel.send(`Loops on, ${message.member.displayName.replace(/@/g, '@\u200b')}, have fun listening to the same stuff *forever*`);
	}
};
