exports.run = (message, args, suffix, client, permLevel) => {
	...
	if (!message.member.roles.cache.find(r => r.name.toLowerCase() === i18n.get('struct.music.dj', message).toLowerCase()) && !message.member.roles.find(r => r.name.toLowerCase() === i18n.get('struct.music.music', message).toLowerCase()) && permLevel < 3 && message.guild.music.queue[num - 1].person.id !== message.author.id) return;
