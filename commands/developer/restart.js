...
exports.run = (message, args, suffix, client) => {
	...
	if ((voice || processing) && force === false) {
		return message.channel.send(`I'm not gonna restart. ${voice ? `I've got ${client.voice.connections.size} guild${client.voiceConnections.size !== 1 ? 's' : ''} listening to music through me..` : ''} ${processing ? `${voice ? '\n*and*' : ''} I've got the following things processing:\n${client.processing.map(p => `\`${p}\``).join('\n')}` : ''}\n*Bypass with -f*`);
	}
